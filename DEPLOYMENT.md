...
- **Размер файла**: `99 676` байт
- **Контрольная сумма SHA-256**: `27D5887CEF9E5464283BD3A734AF897ED2A5B8CF5D8D73B16A53079467BF52AB`
...
- **Сверка хеш-суммы**: Хеш скопированного в экземпляр JAR полностью совпадает со сборочным артефактом (`27D5887CEF9E5464283BD3A734AF897ED2A5B8CF5D8D73B16A53079467BF52AB`).
...
- Пройдено тестов: **19 из 19** (`BUILD SUCCESSFUL`).
- Проверено:
  ...
  - Уведомление игрока при частичной доставке (`pocketodds.delivery.partial_queued`) без ложных победных эффектов полного получения.
  - Связывание `finished = true` строго с успешным результатом фиксации `finalizeOutcome`.
  - Отсутствие скрытых fallback-замен предметов через фабрику наград `SlotRewardFactory`.
