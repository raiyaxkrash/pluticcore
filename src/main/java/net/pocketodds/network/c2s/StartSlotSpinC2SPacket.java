...
public class StartSlotSpinC2SPacket {
    private final int containerId;
    private final UUID operationId;

    public StartSlotSpinC2SPacket(int containerId, UUID operationId) {
        this.containerId = containerId;
        this.operationId = operationId != null ? operationId : UUID.randomUUID();
    }

    public StartSlotSpinC2SPacket(FriendlyByteBuf buf) {
    ...
    public void handle(Supplier<NetworkEvent.Context> contextSupplier) {
        ...
        context.enqueueWork(() -> {
            ...
            if (player != null && player.containerMenu instanceof PocketCasinoMenu menu
                    ...
                    && menu.getCurrentCategory() == CasinoCategory.SLOTS) {
                CasinoGameService.spinSlots(player, menu, operationId);
            }
        });
